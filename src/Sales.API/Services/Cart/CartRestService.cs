                  ...
                  : Service, ICartRestService
{
    ...
    public async Task<Response<AddItemToCartResponseDTO>> AddItemToCartAsync(CartItensDTO cartItens)
    {
        var response = await _client.PostAsync(string.Empty, GetContent(cartItens)).ConfigureAwait(false);

        var result = await DeserializeObjectResponse<Response<AddItemToCartResponseDTO>>(response);
